...
<html>
    ...
    {
        $fcq=  mysql_query("select course_duration from d_students where reg_num='".$_POST['regn']."' and duration_unit='month' and record_status='A' limit 1",$conn);
        $fcr= mysql_fetch_array($fcq);
         $chkrcpt=  mysql_query("select count(*) as c from d_fee_payments where student_reg_num='".$_POST['regn']."' and fee_type='tuition' and record_status='A'",$conn);
    $chkrcptr=  mysql_fetch_array($chkrcpt);
    /*if((int)$chkrcptr['c']>=(int)$fcr['course_duration']){*/
    $ridq=  mysql_query("select get_Trcpt_id() as rid",$conn);
...
$_POST['admission_fee']."','".$_POST['fee_paid']."','tuition','".$_POST['payment_date']."','".$_POST['nextinst']."','".
 ...
 "A',now())";
        ...
        if($rqq)
        {
            header("location:tuitionfee.php?rcptnum=".$ridr['rid']);
        }
 else {header("location:tuitionfee.php?info=Something Went Wrong while submitting");}
 /*}
    else
    {
        header("location:tuitionfee.php?info=All Tuition Fees Paid for this Student - ".$fcr['course_duration']." - ".$chkrcptr['c']);
    }*/
    }
    ?>
    ...
    <body>
       ...
       <span style="font-family: 'calibri'; color:#bdbdbd; font-size: 50px">Tuition Fee Payment </span> 
    <center>
        <div class="bodytext">
        <br><br>
        ...
        <div class="bodytext" style="border:1px solid #dddddd;border-radius:5px">
            ...
            <table cellspacing="0" cellpadding="5" border="0">
                ...
                <tr>
                    <td>Tuition Fee</td><td><input type="text" class="txtbx" id="fee_paid" name="fee_paid"><input type="hidden" id="admission_fee" name="admission_fee"></td>
                ...
                <tr>
                    ...
                    <td></td><td><input type="hidden" class="txtbx" id="classstart" name="classstart">
                    <input type="hidden" class="txtbx" id="cname" name="cname"><input type="hidden" id="ccd" name="ccd">
                    <input type="hidden" class="txtbx" id="c_full_duration" name="c_full_duration"><input type="hidden" id="c_duration" name="c_duration"><input type="hidden" id="duration_unit" name="duration_unit">
                    <input type="hidden" class="txtbx" id="sfn" name="sfn">
                    <input type="hidden" class="txtbx" id="lc" name="lc">
                    </td>
...
FROM `d_students` where record_status='A' and reg_num=".$_GET['regnum'],$conn);
    ...
    <script type="text/javascript">
        ...
        document.getElementById('fee_paid').value="<?php echo $pr['tuition_fee']?>";
...
if(isset($_GET['rcptnum']))
{
    ?>
        <script>
        if(confirm('Receipt:<?php echo $_GET['rcptnum'];?> Submitted Successfully \n\n Do You want to Generate PDF receipt?'))
        {
            document.location="trcptprint.php?rcptnum=<?php echo $_GET['rcptnum'];?>";
        }
        </script>
